...
class Cell:
    ...
    def draw(self, sc):
        ...
        if self.walls['bottom']:
            pygame.draw.line(sc, pygame.Color('black'), (x, y + TILE), (x, y + TILE), self.thickness)
        if self.walls['left']:
    ...
    def get_rects(self):
        ...
        if self.walls['right']:
            rects.append(pygame.Rect((x + TILE - self.thickness, y), (self.thickness, TILE)))
        if self.walls['bottom']:
            rects.append(pygame.Rect((x, y + TILE - self.thickness), (TILE, self.thickness)))
        if self.walls['left']:
    ...
    def check_neighbors(self, grid_cells, level=1):
        neighbors = []
...
def generate_maze(level=1):
    ...
    while break_count != len(grid_cells):
        ...
        next_cell = current_cell.check_neighbors(grid_cells, level)
        if next_cell:
